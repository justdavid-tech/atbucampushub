...
function Academics() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        setLoading(true);
        const data = await getPostsByCategory('academics');
        setPosts(data);
      } catch (err) {
        console.error("Failed to fetch academic posts:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      ...
      <section className="relative bg-gradient-to-br from-[hsl(var(--background))] via-[hsl(210,45%,14%)] to-[hsl(var(--background))] overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-5">
          <GraduationCap className="w-[600px] h-[600px] text-white animate-float" strokeWidth={0.5} />
        </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-6 animate-fadeIn">
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold leading-tight">
                ...
                <span className="text-white">At ATBU</span>
              </h1>

              {/* Description */}
              <p className="text-lg text-gray-300 max-w-xl leading-relaxed">
                Stay informed on academic calendars, course registrations, examination schedules,
                learning resources, and faculty announcements. All in one place.
              </p>
              ...
              <div className="flex items-center gap-4 text-sm text-gray-400 pt-4">
                <div className="flex items-center gap-2">
                  ...
                  <span>Updated Per Semester</span>
                </div>
                <div className="w-1 h-1 bg-gray-600 rounded-full"></div>
                <div className="flex items-center gap-2">
                  <GraduationCap className="w-4 h-4 text-[hsl(var(--secondary))]" />
                  <span>All Faculties Covered</span>
                </div>
              </div>
            </div>
            ...
            <div className="relative flex items-center justify-center">
              <div className="relative w-72 h-72 rounded-full bg-gradient-to-br from-[hsl(var(--primary))]/10 to-[hsl(var(--secondary))]/10 border border-white/10 flex items-center justify-center backdrop-blur-sm animate-float">
                <div className="w-36 h-36 rounded-full bg-gradient-to-br from-[hsl(var(--primary))] to-[hsl(var(--secondary))] flex items-center justify-center shadow-2xl">
                  <GraduationCap className="w-18 h-18 text-white" />
                </div>
              </div>
            </div>
          </div>
            ...
            {filteredPosts.map((post, index) => (
              ...
              >
                ...
                <div className="relative h-48 overflow-hidden">
                  <img 
                    src={post.imageUrl || 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?w=800&auto=format&fit=crop'} 
                    alt={post.title}
                  ...
                  <div className="absolute top-4 left-4">
                    <span className="inline-flex items-center gap-2 px-3 py-1 bg-[hsl(var(--primary))]/90 backdrop-blur-md rounded-full text-xs font-bold text-white">
                      ...
                      {post.department === 'all' ? 'All Departments' : post.department || 'Academic'}
                    </span>
                  </div>
                </div>
